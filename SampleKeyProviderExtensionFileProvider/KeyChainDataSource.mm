...
- (id) initWithMode:(enum KeyChainDataSourceMode)kcdsMode password:(NSString*)s;
{
    self = [super init];
    if(self)
    {
...
- (NSString*) getNotBeforeAt:(long)index;
{
    SecCertificateRef cert = [self getCertificateAt:index];
    if(!cert)return nil;
    ...
    CFRelease(cfData);
    if (certificateX509 != NULL) {
        ASN1_TIME *notBefore = X509_get_notBefore(certificateX509);
        BIO* bmem = BIO_new(BIO_s_mem());
        ASN1_TIME_print(bmem, notBefore);
        BUF_MEM* bptr = NULL;
        BIO_get_mem_ptr(bmem, &bptr);
        std::string time(bptr->data, bptr->length);
        NSString* retval = [NSString stringWithCString:time.c_str() encoding:NSUTF8StringEncoding];
        BIO_free_all(bmem);
...
//    //Put the PKCS 12 password on the pasteboard
    ...
    s = [NSMutableString stringWithString:self.password];
